...
DISCOUNT_FACTOR = 0.99 # i.e. no discount, we get the total reward to the end
...
NUM_EPISODES = 200000
NUM_REPEATS = 1
NUM_STATS_EPISODES = 20
NUM_EPISODES_PER_UPDATE = 4

MINIBATCH_SIZE = 1
GCN_HIDDEN_NEURONS = 8 # hidden neurons during convolution
GCN_OUTPUT_NEURONS = 8 # how many neurons for a single node representation after convolving
GRAPH_POOLING_NEURONS = 256 # how many neurons for a whole graph representation
...
#execution_features = ["PAPI_TOT_INS","PAPI_TOT_CYC"]
#execution_features = ["DURATION"]
...
#benchmark = "/home/rneill/workspace/projects/openstream/examples/jacobi-1d/stream_df_jacobi_1d_GPU -s x26 -b x22 -r 1"
...
for episode_idx in range(NUM_EPISODES):
	...
	while termination == False:
		...
		replay_memory.add(Transition(pre_state, action, new_state, reward, termination, actionable_nodes_at_pre_state=actionable_nodes_at_current_state,action_probability=probabilities))
		replay_memory_stats.add(Transition(pre_state, action, new_state, reward, termination, actionable_nodes_at_pre_state=actionable_nodes_at_current_state))
	...
	if taking_stats == False and (episode_idx > 0) and (episode_idx % NUM_EPISODES_PER_UPDATE == 0):
		for start_index in range(0, len(episode_replay), MINIBATCH_SIZE):
			...
			loss = policy_network.fit_to_minibatch(sess, features_per_transition, actions, rewards, actionable_nodes_per_transition, None, None)
			logging.debug("Loss for transition " + str(start_index) + ":" + str(loss))
